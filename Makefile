...
OVMS_MEDIA_DOCKER_IMAGE ?= ovms_geti_mediapipe
...
run_holistic_tracking:
	...
	bazel-bin/external/mediapipe/mediapipe/examples/desktop/holistic_tracking/holistic_tracking_cpu --calculator_graph_config_file /mediapipe/mediapipe/graphs/holistic_tracking/holistic_tracking_cpu.pbtxt --input_video_path=/mediapipe/video.mp4 --output_video_path=/mediapipe/output_holistic_ovms.mp4

run_face_detection:
	echo "Running FPS test for face_detection demo"
	rm -rf /mediapipe/output_face_detection_ovms.mp4
	if [ ! -f video.mp4 ]; then wget -O video.mp4 $(INPUT_VIDEO_LINK); fi
	bazel-bin/external/mediapipe/mediapipe/examples/desktop/face_detection/face_detection_cpu --calculator_graph_config_file /mediapipe/mediapipe/graphs/face_detection/face_detection_desktop_live.pbtxt --input_video_path=/mediapipe/video.mp4 --output_video_path=/mediapipe/output_face_detection_ovms.mp4

run_iris_tracking:
	echo "Running FPS test for iris_tracking demo"
	rm -rf /mediapipe/output_iris_tracking_ovms.mp4
	if [ ! -f video.mp4 ]; then wget -O video.mp4 $(INPUT_VIDEO_LINK); fi
	bazel-bin/external/mediapipe/mediapipe/examples/desktop/iris_tracking/iris_tracking_cpu --calculator_graph_config_file /mediapipe/mediapipe/graphs/iris_tracking/iris_tracking_cpu.pbtxt --input_video_path=/mediapipe/video.mp4 --output_video_path=/mediapipe/output_iris_tracking_ovms.mp4

run_pose_tracking:
	echo "Running FPS test for pose_tracking demo"
	rm -rf /mediapipe/output_pose_track_ovms.mp4
	if [ ! -f video.mp4 ]; then wget -O video.mp4 $(INPUT_VIDEO_LINK); fi
	bazel-bin/external/mediapipe/mediapipe/examples/desktop/pose_tracking/pose_tracking_cpu --calculator_graph_config_file /mediapipe/mediapipe/graphs/pose_tracking/pose_tracking_cpu.pbtxt --input_video_path=/mediapipe/video.mp4 --output_video_path=/mediapipe/output_pose_track_ovms.mp4
