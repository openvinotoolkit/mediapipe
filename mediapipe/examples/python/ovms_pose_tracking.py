...
class RequestingThread(threading.Thread):
    ...
    def run(self):
        ...
        global force_exit
        ovms_pose_tracking = mp.solutions.ovms_pose_tracking
        with ovms_pose_tracking.OvmsPoseTracking() as ovms_pose_tracking:
            while (True):
                self.wait_for_input()
                if force_exit:
                ...
                result = ovms_pose_tracking.process(image)
...
parser.add_argument('--output_video_path', required=False, default="pose_output.mp4", type=str, help='Output path to a video file')
