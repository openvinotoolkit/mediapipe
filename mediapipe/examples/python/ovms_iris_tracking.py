...
class RequestingThread(threading.Thread):
    ...
    def run(self):
        ...
        global force_exit
        ovms_iris_tracking = mp.solutions.ovms_iris_tracking
        with ovms_iris_tracking.OvmsIrisTracking() as ovms_iris_tracking:
            while (True):
                self.wait_for_input()
                if force_exit:
                ...
                result = ovms_iris_tracking.process(image)
...
parser.add_argument('--output_video_path', required=False, default="iris_output.mp4", type=str, help='Output path to a video file')
