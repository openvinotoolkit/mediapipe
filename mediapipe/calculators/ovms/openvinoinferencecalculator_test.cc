...
TEST_F(OpenVINOInferenceCalculatorTest, RestartInference) {
    std::string graph_proto = R"(
      input_stream: "input"
      output_stream: "output"
      node {
...
std::shared_ptr<mediapipe::ThreadPoolExecutor> sharedThreadPool = std::make_shared<mediapipe::ThreadPoolExecutor>(std::thread::hardware_concurrency());
    CalculatorGraphConfig graph_config =
        ParseTextProtoOrDie<CalculatorGraphConfig>(graph_proto);
    const std::string inputStreamName = "input";
    const std::string outputStreamName = "output";
    ...
    ::mediapipe::CalculatorGraph graph;
    MP_ASSERT_OK(graph.SetExecutor("", sharedThreadPool));
    EXPECT_EQ(graph.Initialize(graph_config).code(), absl::StatusCode::kOk);
    MP_ASSERT_OK(graph.StartRun({}));
    auto datatype = ov::element::Type_t::f32;
    ov::Shape shape{1,10};
    ...
    // Start again - simulation of graph pool
    for (auto i =0; i < 100; ++i) {
    output_packets.clear();
    MP_ASSERT_OK(graph.StartRun({}));
    inputTensor = std::make_unique<ov::Tensor>(datatype, shape, data.data());
    MP_ASSERT_OK(graph.AddPacketToInputStream(
        inputStreamName, Adopt(inputTensor.release()).At(Timestamp(0))));
    MP_ASSERT_OK(graph.CloseInputStream(inputStreamName));
    MP_ASSERT_OK(graph.WaitUntilIdle());
    ...
    ASSERT_EQ(1, output_packets.size());
    const ov::Tensor& outputTensor2 =
        output_packets[0].Get<ov::Tensor>();
    MP_ASSERT_OK(graph.WaitUntilDone());
    EXPECT_EQ(datatype, outputTensor2.get_element_type());
    EXPECT_THAT(outputTensor2.get_shape(), testing::ElementsAre(1,10));
    outputData = outputTensor2.data();
    for (size_t i = 0; i < data.size(); ++i) {
        EXPECT_EQ(data[i] + 1, *(reinterpret_cast<const float*>(outputData) + i)) << i;
    }
    }
