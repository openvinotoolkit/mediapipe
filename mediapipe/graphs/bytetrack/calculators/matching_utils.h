...
inline AssignmentResult LinearAssignment(const Eigen::MatrixXf& cost, float thresh){
    ...
    if (N == 0 || M == 0) {
        ...
        for (int i = 0; i < N; ++i) u_tracks(i) = i;
        for (int j = 0; j < M; ++j) u_boxes(j)  = j;
    ...
    for (int j = 0; j < S; ++j) {
        ...
        for (int i = 1; i < S; ++i) {
            if (cost_sq(i, j) < best_v) { best_v = cost_sq(i, j); best_i = i; }
        }
        v[j] = best_v;
        if (row2col[best_i] == -1) {
            row2col[best_i] = j;
    ...
    for (int i_start = 0; i_start < S; ++i_start) {
        ...
        for (int iter = 0; iter < S; ++iter) {
            ...
            d_min = INF;
            for (int j = 0; j < S; ++j)
                if (!visited[j] && dist[j] < d_min) { d_min = dist[j]; j_min = j; }

            if (j_min == -1) break;
            visited[j_min] = true;

            if (col2row[j_min] == -1) { j_end = j_min; break; }
            ...
            for (int j = 0; j < S; ++j) {
                ...
                if (nd < dist[j]) { dist[j] = nd; pred[j] = j_min; }
            }
        }

        // Update col duals along the path
        for (int j = 0; j < S; ++j)
            if (visited[j]) v[j] += dist[j] - d_min;
        u[i_start] += d_min;
