    ...
    absl::Status ByteTrackCalculator::Process(CalculatorContext* cc){
        ...
        if (!cc->Inputs().Tag("DETECTIONS").IsEmpty()) {
            ...
            for (const auto& detection : input) {
                ...
                if (score >= track_high_thresh_) high_dets->push_back(detection);
                else if (score >= track_low_thresh_) low_dets->push_back(detection);
            }
        }
        ...
        for(auto& t : lost_stracks_) lost_ptrs.push_back(&t);
        ...
        // joint_stracks(tracked_stracks_, activated_stracks)
        {
            ...
            for (auto& t : tracked_stracks_)  cur_ptrs.push_back(&t);
            for (auto& t : activated_stracks) act_ptrs.push_back(&t);
            for (auto& t : refind_stracks)    ref_ptrs.push_back(&t);
            ...
            std::vector<bytetrack::STrack> joined_tracks;
             ...
             for (auto* t : joined) joined_tracks.push_back(*t);
        ...
        // sub_stracks(lost_stracks_, tracked_stracks_)  then extend with lost_stracks (local)
        {
            ...
            for (auto& t : lost_stracks_)    lost_ptrs2.push_back(&t);
            for (auto& t : tracked_stracks_) new_tracked_ptrs.push_back(&t); // fresh pointers!
            for (auto& t : removed_stracks_) removed_ptrs.push_back(&t);
            for (auto& t : lost_stracks)     local_lost_ptrs.push_back(&t);
            ...
            for (auto* t : new_lost) l_stracks.push_back(*t);
            lost_stracks_ = std::move(l_stracks);
        }

        // extend removed_stracks_
        for (auto& t : removed_stracks) removed_stracks_.push_back(t);

        // remove duplicates
        {
            std::vector<bytetrack::STrack*> tracked_ptrs2, lost_ptrs3;
            for (auto& t : tracked_stracks_) tracked_ptrs2.push_back(&t);
            for (auto& t : lost_stracks_)    lost_ptrs3.push_back(&t);
            ...
            for (auto* t : dedup_tracked) new_tracked.push_back(*t);
            for (auto* t : dedup_lost)    new_lost.push_back(*t);
        ...
        // output.release(), cc->InputTimestamp());
            cc->Outputs().Get("DETECTIONS", 0).Add(
    output.release(), cc->Inputs().Get("DETECTIONS", 0).Value().Timestamp());
        return absl::OkStatus();
